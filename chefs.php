<?php session_start(); ?>
<!doctype html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="">
    <meta name="author" content="Mark Otto, Jacob Thornton, and Bootstrap contributors">
    <meta name="generator" content="Hugo 0.79.0">
    <title>Chefs</title>

    <link rel="canonical" href="https://getbootstrap.com/docs/5.0/examples/album/">
    <link href="css/chefs.css" rel="stylesheet">
    <link href="css/bootstrap.min.css" rel="stylesheet">
    <script src="fontawesome-free-5.15.1-web/js/all.js"></script>
    <script src="https://kit.fontawesome.com/a076d05399.js"></script>

    <style>
        * {
            font-family: montserrat;

        }

        a {
            text-decoration: none;
        }

        .bd-placeholder-img {
            font-size: 1.125rem;
            text-anchor: middle;
            -webkit-user-select: none;
            -moz-user-select: none;
            user-select: none;
        }

        @media (min-width: 768px) {
            .bd-placeholder-img-lg {
                font-size: 3.5rem;
            }
        }

        .btn.btn-sm.btn-outline-secondary {
            color: white;
            background-color: black;
            border: transparent 0px;

        }

        .btn.btn-sm.btn-outline-secondary:hover {
            background-color: #1b9bff;
        }

        .col-lg-3.col-md-6.col-6 {
            margin-bottom: 25px;
        }

        .col-lg-6.col-md-6.col-12 {
            margin-bottom: 25px;
        }

        .col-lg-3.col-md-6.col-6:hover {
            transform: scale(1.05);
        }

        .card-text {
            text-align: center;
        }

        .card-text a {
            font-weight: bolder;
            font-size: x-large;
        }

        #hh {
            float: left;
            margin-top: 10px;
        }

        .d-flex input {
            height: 40px;
            width: 370px;
        }

        .d-flex {
            float: right;
            margin-right: -15px;
        }

        .d-flex button {
            background-color: black;
            border: black solid 1px;
            color: white;
            height: 40px;

        }

        .d-flex button:hover {
            background-color: #1b9bff;
        }

        @media (max-width:772px) {
            .pagination.flex-wrap {
                flex-wrap: wrap-reverse;
            }
        }

        @media (max-width:767px) {
            .d-flex input {
                width: 425px;
            }
        }

        @media (max-width:500px) {

            #first {
                margin-top: -25px;
            }

            .d-flex {
                margin-top: -25px;
            }

            .d-flex input {
                width: 370px;
            }
        }

        .pagination-wrapper {
            font-size: 0;
            position: absolute;
            top: 50%;
            left: 50%;
            transform: translate(-50%, -50%);
            text-align: center;
        }

        @keyframes pagination-container--animation-prev {
            0% {
                transform: translateX(0);
            }

            100% {
                transform: translateX(18px);
            }
        }

        @keyframes pagination-container--animation-next {
            0% {
                transform: translateX(0);
            }

            100% {
                transform: translateX(-18px);
            }
        }

        .transition-prev .pagination-container {
            animation: pagination-container--animation-prev 0.3s forwards;
        }

        .transition-next .pagination-container {
            animation: pagination-container--animation-next 0.3s forwards;
        }

        .little-dot {
            width: 6px;
            height: 6px;
            background: black;
            border-radius: 100%;
            display: inline-block;
            vertical-align: middle;
            margin: 0 6px;
            position: relative;
            z-index: 10;
        }

        .little-dot--first,
        .little-dot--last {
            z-index: 5;
        }

        @keyframes slideLeft {
            0% {
                transform: translateX(0px);
            }

            100% {
                transform: translateX(-18px);
            }
        }

        .transition-prev .little-dot--first {
            animation: slideLeft 0.4s 0.3s forwards cubic-bezier(0.165, 0.84, 0.44, 1);
        }

        @keyframes little-dot--first--animation {
            0% {
                opacity: 1;
            }

            100% {
                opacity: 0;
            }
        }

        .transition-next .little-dot--first {
            animation: little-dot--last--animation 0.3s forwards;
        }


        @keyframes little-dot--last--animation {
            0% {
                opacity: 1;
            }

            100% {
                opacity: 0;
            }
        }

        .transition-prev .little-dot--last {
            animation: little-dot--last--animation 0.3s forwards;
        }

        @keyframes slideRight {
            0% {
                transform: translateX(0px);
                opacity: 1;
            }

            100% {
                transform: translateX(18px);
                opacity: 1;
            }
        }

        .transition-next .little-dot--last {
            animation: slideRight 0.4s 0.3s forwards cubic-bezier(0.165, 0.84, 0.44, 1);
        }

        .big-dot {
            width: 12px;
            height: 12px;
            border-radius: 100%;
            background: #1b9bff;
            position: absolute;
            top: 50%;
            right: -6px;
            transform: translateY(-50%);
        }

        .transition-next .big-dot {
            right: auto;
            left: -6px;
        }

        .big-dot-container {
            width: 18px;
            height: 18px;
            border-radius: 100%;
            position: absolute;
            top: 50%;
            right: 3px;
            transform: translateY(-50%);
            z-index: 10;
        }

        .transition-next .big-dot-container {
            right: auto;
            left: 3px;
        }

        @keyframes big-dot-container--animation-prev {
            0% {
                transform: translateY(-50%);
            }

            100% {
                transform: translateY(-50%) rotate(-179deg);
            }
        }

        @keyframes big-dot-container--animation-next {
            0% {
                transform: translateY(-50%);
            }

            100% {
                transform: translateY(-50%) rotate(-181deg);
            }
        }

        .transition-prev .big-dot-container {
            animation: big-dot-container--animation-prev 0.3s forwards;
        }

        .transition-next .big-dot-container {
            animation: big-dot-container--animation-next 0.3s forwards;
        }

        .pagination-wrapper .btn {
            position: absolute;
            top: 50%;
            transform: translateY(-50%);
            fill: #1b9bff;
            cursor: pointer;
            transition: opacity 0.2s;
        }

        .pagination-wrapper .btn:hover {
            background-color: transparent;
        }

        .btn--next {
            left: calc(100% + 20px);
        }

        .btn--prev {
            right: calc(100% + 20px);
        }
    </style>


</head>

<body>

    <header>
        <nav style="position:relative">
            <input type="checkbox" id="check" onclick="">
            <label for="check" class="checkbtn">
                <i class="fas fa-bars"></i>
            </label>
            <label class="logo" onclick="location.href='index.php';">HomeChef</label>
            <ul>
                <li><a class="active" href="index.php">Home</a></li>
                <li><a href="chefs.php">Restaurants</a></li>
                <?php
                if (!isset($_SESSION["type"])) {
                    echo "<li><a target=\"_blank\" href=\"join-us.html\">Join Us</a></li>\n<li><a href=\"login.html\">Login</a></li>";
                } else {
                    if ($_SESSION["type"] == "chef") {
                        echo "<li><a href=\"chef.php?id=" . $_SESSION["userInfo"]["data"]["id"] . "\">View account</a></li>\n<li><a href=\"chef.php?edit\">Edit account details</a></li>";
                    }
                    echo "<li><a href=\"chat.php\">Chat</a></li>\n<li><a href=\"cart.php\">Cart</a></li>\n<li><a href=\"logout.php\">Logout</a></li>
                    <link href=\"css/navtry.css\" rel=\"stylesheet\">";
                }
                ?>
            </ul>
        </nav>
    </header>

    <main>
        <div class="album py-5 bg-light">
            <div class="container" id="cc">
                <!-- begin of row -->
                <div class="row" id="first">
                    <div class="col-lg-6 col-md-6 col-12">
                        <div class="col">
                            <h1 class="card-text" id="hh">All Chefs</h1>
                        </div>
                    </div>
                    <div class="col-lg-6 col-md-6 col-12">
                        <div class="col">
                            <div class="card-body">




                                <form class="d-flex" action="chefs.php?page=1" method="Get">
                                    <?php
                                    if (isset($_GET["searchbar"]) && $_GET["searchbar"] != "") {
                                        $g = $_GET["searchbar"];
                                        echo ' <input name="searchbar" class="form-control me-2" type="search" value=' . $g . ' aria-label="Search">';
                                    } else {
                                        echo ' <input name="searchbar" class="form-control me-2" type="search" placeholder="Search for a chef or a meal ...." aria-label="Search">';
                                    }

                                    ?>
                                    <button class="btn btn-outline-success" type="submit">Search</button>
                                </form>
                            </div>
                        </div>
                    </div>



                    <?php

                    if (!isset($_GET["page"])) {
                        $p = 1;
                    } else {
                        $p = $_GET["page"];
                        if ($p < 1) {
                            $p = 1;
                        }
                    }


                    $limit = 8;
                    $startpoint = ($p * $limit) - $limit;
                    $con = mysqli_connect("localhost", "root", "", "joinus");


                    if (isset($_GET["searchbar"]) && $_GET["searchbar"] != "") {
                        $seaechb = $_GET["searchbar"];
                        if ($_GET["searchbar"] == "offer") {
                            $selectoffer = "select * from joinu where Offer>0  LIMIT {$startpoint} , {$limit} ";
                            $dooffer = mysqli_query($con, $selectoffer) or die(mysqli_error($con));
                            $rowsoffer = mysqli_num_rows($dooffer);
                            $selectoffer2 = "select * from joinu where Offer>0";
                            $do2 = mysqli_query($con, $selectoffer2) or die(mysqli_error($con));
                            $rows2 = mysqli_num_rows($do2);

                            $max = (int)ceil($rows2 / $limit);
                            if ($rowsoffer > 0) {

                                for ($r = 1; $r <= $rowsoffer; $r++) {
                                    $ro = mysqli_fetch_array($dooffer);

                                    searcheditems($ro);
                                }
                            }
                        } else {
                            $selectfirstname = "select * from joinu where (restname LIKE '$seaechb%') OR (category LIKE '$seaechb%')  LIMIT {$startpoint} , {$limit} ";
                            $dofirstname = mysqli_query($con, $selectfirstname) or die(mysqli_error($con));
                            $rowsfirstname = mysqli_num_rows($dofirstname);


                            $selectrows = "select * from joinu where (restname LIKE '$seaechb%') OR (category LIKE '$seaechb%')";
                            $dorows = mysqli_query($con, $selectrows) or die(mysqli_error($con));
                            $rowsnum = mysqli_num_rows($dorows);
                            $max = (int)ceil($rowsnum / $limit);
                            if ($rowsfirstname > 0) {

                                for ($r = 1; $r <= $rowsfirstname; $r++) {
                                    $ro = mysqli_fetch_array($dofirstname);
                                    searcheditems($ro);
                                }
                            }
                        }
                    } else {


                        $selectall = "select * from joinu LIMIT {$startpoint} , {$limit} ";
                        $do = mysqli_query($con, $selectall) or die(mysqli_error($con));
                        $rows = mysqli_num_rows($do);

                        $selectall2 = "select * from joinu";
                        $do2 = mysqli_query($con, $selectall2) or die(mysqli_error($con));
                        $rows2 = mysqli_num_rows($do2);

                        $max = (int)ceil($rows2 / $limit);

                        for ($r = 1; $r <= $rows; $r++) {
                            $ro = mysqli_fetch_array($do);


                            searcheditems($ro);
                        }
                    }



                    function searcheditems($ro)
                    {

                        $photo = $ro["Photo"];
                        $restname = $ro["RestName"];
                        $cat = $ro["Category"];
                        $id = $ro["ID"];

                        echo " 
       
          <div class='col-lg-3 col-md-6 col-6' onclick=" . "location.href='chef.php?id=$id';" . ">
            <div class='col'>
              <div class='card shadow-sm'>
                <img src =$photo>
              
                 <div class='card-body'>
                  <p class='card-text'><a>$restname

                        </a><br>
                    <small>$cat

                        </small>
                  </p>
                  <div class='d-flex justify-content-between align-items-center'>
                    <div class='btn-group'>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
         
         
             
          ";
                    }

                    if ($max == 0) {
                        print ' 
       
          <div class="col-lg-3 col-md-6 col-6" style="height:150px;">
          </div>
         
         
             
          ';
                    }

                    ?>
                </div>


                <!--row end-->
                <div class="row" class="end" style="position: relative;">

                </div>

                <div style="position: relative; z-index: 0; margin-bottom: 80px; margin-top: 50px;">
                    <div class="pagination-wrapper">
                        <?php
                        if ($max > 0) {
                            if (isset($_GET["searchbar"])) {
                                $s = $_GET["searchbar"];
                                $ss = "&searchbar=$s";
                            } else {
                                $ss = "";
                            }
                            if ($p == 1) {
                                echo ' <li><a href="chefs.php?page=' . $p . $ss . '"><svg class="btn btn--prev" height="96" viewBox="0 0 24 24" width="96" xmlns="http://www.w3.org/2000/svg">
                            <path d="M15.41 16.09l-4.58-4.59 4.58-4.59L14 5.5l-6 6 6 6z" />
                            <path d="M0-.5h24v24H0z" fill="none" />
                        </svg></a></li>';
                            } else {
                                echo ' <li><a href="chefs.php?page=' . ($p - 1) . $ss . '"><svg class="btn btn--prev" height="96" viewBox="0 0 24 24" width="96" xmlns="http://www.w3.org/2000/svg">
                            <path d="M15.41 16.09l-4.58-4.59 4.58-4.59L14 5.5l-6 6 6 6z" />
                            <path d="M0-.5h24v24H0z" fill="none" />
                        </svg></a></li>';
                            }


                            echo '<div class="pagination-container">
                            <div class="little-dot  little-dot--first"></div>
                            <div class="little-dot">
                                <div class="big-dot-container">
                                    <div class="big-dot"></div>
                                </div>
                            </div>
                            <div class="little-dot  little-dot--last"></div>
                        </div>';
                            if ($p >= $max) {
                                echo '<li><a href="chefs.php?page=' . $max . $ss . '"><svg class="btn btn--next" height="96" viewBox="0 0 24 24" width="96" xmlns="http://www.w3.org/2000/svg" >
                                <path d="M8.59 16.34l4.58-4.59-4.58-4.59L10 5.75l6 6-6 6z" />
                                <path d="M0-.25h24v24H0z" fill="none" />
                            </svg></a></li>';
                            } else {
                                echo '<li><a href="chefs.php?page=' . ($p + 1) . $ss . '"><svg class="btn btn--next" height="96" viewBox="0 0 24 24" width="96" xmlns="http://www.w3.org/2000/svg" >
                                <path d="M8.59 16.34l4.58-4.59-4.58-4.59L10 5.75l6 6-6 6z" />
                                <path d="M0-.25h24v24H0z" fill="none" />
                            </svg></a></li>';
                            }
                        }

                        ?>



                    </div>
                </div>
                <script src="js/pagination.js"></script>
            </div>
        </div>






        <footer id="footer" class="footer-1">
            <div class="main-footer widgets-dark typo-light">
                <div class="container">
                    <div class="row">

                        <div class="col-xs-12 col-sm-6 col-md-3">
                            <div class="widget subscribe no-box">
                                <h5 class="widget-title">HomeChef<span></span></h5>
                                <p>HomeChef tends to help people especially, Talented one to get a good income from a hobby, an interest, or a profession in cooking. This site try to present chefs to all the world and show their work. and also to help people getting a homemade clean healthy food.</p>
                            </div>
                        </div>

                        <div class="col-xs-12 col-sm-6 col-md-3">
                            <div class="widget no-box">
                                <h5 class="widget-title">Quick Links<span></span></h5>
                                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="index.php">Home</a></span><br>
                                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="chefs.php">Restaurants
                                    </a></span><br>
                                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="join-us.html" target="_blank">Join Us</a></span><br>
                                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="Aboutus.php">About Us</a></span><br>
                            </div>
                        </div>

                        <div class="col-xs-12 col-sm-6 col-md-3">
                            <div class="widget no-box">
                                <h5 class="widget-title">Get Started<span></span></h5>
                                <p>Stop Wasting Your Time and hiding your talent, join us now and share your food with the world and make money with us.</p>
                                <a style="color:white" class="btn" href="join-us.html" target="_blank">Join Us Now</a>
                            </div>
                        </div>

                        <div class="col-xs-12 col-sm-6 col-md-3">

                            <div class="widget no-box">
                                <h5 class="widget-title">Contact Us<span></span></h5>

                                <p><a href="mailto:info@domain.com" title="glorythemes">Gergesabdo99@gmail.com</a></p>
                                <a title="youtube" target="_blank" href="https://www.youtube.com/"><img alt="youtube" width="30" height="30" src="data:image/png;base64,"></a>

                                <a href="https://www.facebook.com/" target="_blank" title="Facebook"><img alt="Facebook" width="30" height="30" src="data:image/png;base64,"></a>

                                <a href="https://twitter.com" target="_blank" title="Twitter"><img alt="Twitter" width="30" height="30" src="data:image/png;base64,"></a>

                                <a title="instagram" target="_blank" href="https://www.instagram.com/"><img alt="instagram" width="30" height="30" src="data:image/png;base64,"></a>
                            </div>
                        </div>

                    </div>
                </div>
            </div>

            <div class="footer-copyright">
                <div class="container">
                    <div class="row">
                        <div class="col-md-12 text-center">
                            <p>Copyright HomeChef © 2021. All rights reserved.</p>
                        </div>
                    </div>
                </div>
            </div>
        </footer>
    </main>


    <script src="js/bootstrap.bundle.min.js"></script>


</body>

</html>
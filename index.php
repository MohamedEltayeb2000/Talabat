<!doctype html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="">
    <meta name="author" content="Mark Otto, Jacob Thornton, and Bootstrap contributors">
    <meta name="generator" content="Hugo 0.79.0">
    <title>First Page</title>
    <script src="fontawesome-free-5.15.1-web/js/all.js"></script>
    <link href="css/style.css" rel="stylesheet">
    <link href="css/bootstrap.min.css" rel="stylesheet">

    <style>
        .spinner {
            width: 40px;
            height: 40px;
            background-color: white;

            position: absolute;
            top: 48%;
            left: 48%;
            -webkit-animation: sk-rotateplane 1.2s infinite ease-in-out;
            animation: sk-rotateplane 1.2s infinite ease-in-out;
        }

        @-webkit-keyframes sk-rotateplane {
            0% {
                -webkit-transform: perspective(120px)
            }

            50% {
                -webkit-transform: perspective(120px) rotateY(180deg)
            }

            100% {
                -webkit-transform: perspective(120px) rotateY(180deg) rotateX(180deg)
            }
        }

        @keyframes sk-rotateplane {
            0% {
                transform: perspective(120px) rotateX(0deg) rotateY(0deg);
                -webkit-transform: perspective(120px) rotateX(0deg) rotateY(0deg)
            }

            50% {
                transform: perspective(120px) rotateX(-180.1deg) rotateY(0deg);
                -webkit-transform: perspective(120px) rotateX(-180.1deg) rotateY(0deg)
            }

            100% {
                transform: perspective(120px) rotateX(-180deg) rotateY(-179.9deg);
                -webkit-transform: perspective(120px) rotateX(-180deg) rotateY(-179.9deg);
            }
        }

        .spinner-wrapper {
            position: fixed;
            top: 0;
            left: 0;
            right: 0;
            bottom: 0;
            background-color: black;
            z-index: 999999;
        }
        a{
            text-decoration:none;
        }
    </style>
</head>

<body>
    <div class="spinner-wrapper">
        <div class="spinner"></div>
    </div>
    <header>
        <nav style="position:absolute;">
            <input type="checkbox" id="check" onclick="">
            <label for="check" class="checkbtn">
                <i class="fas fa-bars"></i>
            </label>
            <label class="logo" onclick="location.href='index.php';">HomeChef</label>
            <ul>
                <li><a class="active" href="index.php">Home</a></li>
                <li><a href="chefs.php">Restaurants</a></li>
                <?php
                session_start();
                if (!isset($_SESSION["type"])) {
                    echo "<li><a target=\"_blank\" href=\"join-us.html\">Join Us</a></li>\n<li><a href=\"login.html\">Login</a></li>";
                } else {
                    if ($_SESSION["type"] == "chef") {
                        echo "<li><a href=\"chef.php?id=" . $_SESSION["userInfo"]["data"]["id"] . "\">View account</a></li>\n<li><a href=\"chef.php?edit\">Edit account details</a></li>";
                    }
                    echo "<li><a href=\"chat.php\">Chat</a></li>\n<li><a href=\"cart.php\">Cart</a></li>\n<li><a href=\"logout.php\">Logout</a></li>
                    <link href=\"css/navtry.css\" rel=\"stylesheet\">";
                }
                ?>
                <!-- <li><a href="#..">Feedback</a></li> -->
            </ul>
        </nav>
    </header>
    <main style="vertical-align: top;">
        <div id="myCarousel" class="carousel slide" data-bs-ride="carousel">
            <ol class="carousel-indicators">
                <li data-bs-target="#myCarousel" data-bs-slide-to="0" class="active"></li>
                <li data-bs-target="#myCarousel" data-bs-slide-to="1"></li>
                <li data-bs-target="#myCarousel" data-bs-slide-to="2"></li>
            </ol>
            <div class="carousel-inner">
                <div class="carousel-item active">
                    <img src="img/healthy-green-vegetarian-buddha-bowl-lunch-with-eggs-rice-tomato-avocado-blue-cheese-table.jpg" class="img-fluid" width=1450 height=750>
                    <rect width="100%" height="100%" fill="#777" />
                </div>
                <div class="carousel-item">
                    <img src="img/indian-dhal-spicy-curry-bowl-spices-herbs-rustic-black-wooden-table.jpg" class="img-fluid" width=1450 height=750>
                    <rect width="100%" height="100%" fill="#777" />
                </div>
                <div class="carousel-item">
                    <img src="img/chicken-wings-barbecue-sweetly-sour-sauce-picnic-summer-menu-tasty-food-top-view-flat-lay.jpg" class="img-fluid" width=1450 height=750>
                    <rect width="100%" height="100%" fill="#777" />
                </div>
            </div>
            <a class="carousel-control-prev" href="#myCarousel" role="button" data-bs-slide="prev">
                <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                <span class="visually-hidden">Previous</span>
            </a>
            <a class="carousel-control-next" href="#myCarousel" role="button" data-bs-slide="next">
                <span class="carousel-control-next-icon" aria-hidden="true"></span>
                <span class="visually-hidden">Next</span>
            </a>
            <div class="container">
                <div class="carousel-caption text-start">
                    <form class="d-flex" action="chefs.php?page=1" method="Get">
                        <input input name="searchbar" class="form-control me-2" type="search" placeholder="Search for a chef or a meal ...." aria-label="Search">
                        <button class="btn btn-outline-success" type="submit">Search</button>
                    </form>
                </div>
            </div>
        </div>


        <!-- Marketing messaging and featurettes
  ================================================== -->
        <!-- Wrap the rest of the page in another container to center all the content. -->

        <div class="container marketing">

            <!-- two cards of text below the carousel -->
            <h1>Categories</h1><br>
            <div class="row justify-content-center">
                <div class="col-lg-6" onclick="location.href='chefs.php?searchbar=sweet'">
                    <div class="card mb-3" style="max-width: 540px;">
                        <div class="row no-gutters">
                            <div class="col-md-4">
                                <img style="height:100%" src="img/featured-restaurant-food-thumb-img3.jpg" class="card-img" alt="...">
                            </div>
                            <div class="col-md-8">
                                <div class="card-body">
                                    <h4 class="card-title">sweet</h4>
                                    <p class="card-text">Find best chefs that offers best deserts.</p>
                                    <p class="card-text"><small><a href="#...">Explore</a></small></p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <!-- /.col-lg-6 -->
                <div class="col-lg-6" onclick="location.href='chefs.php?searchbar=salty'">
                    <div class="card mb-3" style="max-width: 540px;">
                        <div class="row no-gutters">
                            <div class="col-md-4">
                                <img style="height:100%" src="img/popular-post-img2-4.jpg" class="card-img" alt="...">
                            </div>
                            <div class="col-md-8">
                                <div class="card-body">
                                    <h4 class="card-title">Salty</h4>
                                    <p class="card-text">Find best chefs that offers best home meals.</p>
                                    <p class="card-text"><small><a href="#...">Explore</a></small></p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <!-- /.col-lg-6 -->

            </div>
            <br>
            <div class="row">
                <div class="col-lg-6" onclick="location.href='chefs.php?searchbar=sweet & salty'">
                    <div class="card mb-3" style="max-width: 540px;">
                        <div class="row no-gutters">
                            <div class="col-md-4">
                                <img style="height:100%" src="img/featured-restaurant-food-thumb-img3.jpg" class="card-img" alt="...">
                            </div>
                            <div class="col-md-8">
                                <div class="card-body">
                                    <h4 class="card-title">sweet & salty</h4>
                                    <p class="card-text">Find best chefs that offers best deserts.</p>
                                    <p class="card-text"><small><a href="#...">Explore</a></small></p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <!-- /.col-lg-6 -->

            </div>
            <!-- /.row -->


            <!-- offers slider-->

            <?php
            $limit = 6;
            $con = mysqli_connect("localhost", "root", "", "joinus");
            $selectoffer = "select * from joinu where Offer>0  LIMIT $limit ";
            $dooffer = mysqli_query($con, $selectoffer) or die(mysqli_error($con));
            $rowsoffer = mysqli_num_rows($dooffer);
            if ($rowsoffer > 0) {
                print '<hr class="featurette-divider">
            <h1 style="text-align: left;">Our Best Deals</h1>
            <a href="chefs.php?searchbar=offer" style="text-align: right; float:right; margin-top: -50px; font-size: 20px;" id="view">View
                All</a><br>';
                print '<div class="row">';
                for ($r = 1; $r <= $rowsoffer; $r++) {
                    $ro = mysqli_fetch_array($dooffer);

                    offersitems($ro);
                    if ($r % 2 == 0) {
                        print '<br>';
                    }
                }
            }

            function offersitems($ro)
            {
                $photo = $ro["Photo"];
                $id = $ro["ID"];
                $restname = $ro["RestName"];
                $ad = $ro["Address"];
                $offer = $ro["Offer"];
                echo "
                        		
                        		 <div class='col-lg-6' onclick=" . "location.href='chef.php?id=$id';" . ">
                    <div class='card mb-3' style='max-width: 540px;'>
                        <div class='row no-gutters'>
                            <div class='col-md-4'>
                                <img style='height:100%' src=$photo
                               class='card-img' alt='...'>
                            </div>
                            <div class='col-md-8'>
                                <div class='card-body'>
                                    <h4 class='card-title'><a href='#...'>$restname
                                    </a></h4>
                                    <p class='card-text'> $ad
                                    </p>
                                    <p class='card-text2'><small><a href='#...' class='percent'>$offer%</a></small>
                                    </p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
    			
                        	";
            }
            print '</div>';

            ?>


            <!-- /.row -->


            <hr class="featurette-divider">
            <h1 style="text-align:left; padding-left: 8px;">Famous Places</h1><br>
            <div class="container-fluid">
                <div class="map-responsive">
                    <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3410.944305322001!2d29.973031214981575!3d31.249965081459912!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x14f5dacb5c541f11%3A0x789ff5db20f2d663!2sMcDonald&#39;s!5e0!3m2!1sen!2seg!4v1607983496761!5m2!1sen!2seg" width="650px" height="450" frameborder="0" style="border:0;"></iframe>
                </div>
            </div>

            <hr class="featurette-divider">

            <!-- /END THE FEATURETTES -->

        </div>
        <!-- /.container -->
        <footer id="footer" class="footer-1">
            <div class="main-footer widgets-dark typo-light">
                <div class="container">
                    <div class="row">

                        <div class="col-xs-12 col-sm-6 col-md-3">
                            <div class="widget subscribe no-box">
                                <h5 class="widget-title">HomeChef<span></span></h5>
                                <p>HomeChef tends to help people especially, Talented one to get a good income from a hobby, an interest, or a profession in cooking. This site try to present chefs to all the world and show their work. and also to help people getting a homemade clean healthy food.</p>
                            </div>
                        </div>

                        <div class="col-xs-12 col-sm-6 col-md-3">
                            <div class="widget no-box">
                                <h5 class="widget-title">Quick Links<span></span></h5>
                                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="index.php">Home</a></span><br>
                                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="chefs.php">Restaurants
                                    </a></span><br>
                                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="join-us.html" target="_blank">Join Us</a></span><br>
                                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="Aboutus.php">About Us</a></span><br>
                            </div>
                        </div>

                        <div class="col-xs-12 col-sm-6 col-md-3">
                            <div class="widget no-box">
                                <h5 class="widget-title">Get Started<span></span></h5>
                                <p>Stop Wasting Your Time and hiding your talent, join us now and share your food with the world and make money with us.</p>
                                <a style="color:white" class="btn" href="join-us.html" target="_blank">Join Us Now</a>
                            </div>
                        </div>

                        <div class="col-xs-12 col-sm-6 col-md-3">

                            <div class="widget no-box">
                                <h5 class="widget-title">Contact Us<span></span></h5>

                                <p><a style="color:white" href="mailto:info@domain.com" title="glorythemes">Gergesabdo99@gmail.com</a></p>
                                <a title="youtube" target="_blank" href="https://www.youtube.com/"><img alt="youtube" width="30" height="30" src="data:image/png;base64,"></a>

                                <a href="https://www.facebook.com/" target="_blank" title="Facebook"><img alt="Facebook" width="30" height="30" src="data:image/png;base64,"></a>

                                <a href="https://twitter.com" target="_blank" title="Twitter"><img alt="Twitter" width="30" height="30" src="data:image/png;base64,"></a>

                                <a title="instagram" target="_blank" href="https://www.instagram.com/"><img alt="instagram" width="30" height="30" src="data:image/png;base64,"></a>
                            </div>
                        </div>

                    </div>
                </div>
            </div>

            <div class="footer-copyright">
                <div class="container">
                    <div class="row">
                        <div class="col-md-12 text-center">
                            <p>Copyright HomeChef © 2021. All rights reserved.</p>
                        </div>
                    </div>
                </div>
            </div>
        </footer>
    </main>


    <script src="js/bootstrap.bundle.min.js"></script>
    <script>
        function sleep(milliseconds) {
            const date = Date.now();
            let currentDate = null;
            do {
                currentDate = Date.now();
            } while (currentDate - date < milliseconds);
        }
        let spinnerWrapper = document.querySelector('.spinner-wrapper');

        window.addEventListener('load', function() {
            sleep(3000);
            spinnerWrapper.parentElement.removeChild(spinnerWrapper);
        });
    </script>

</body>


</html>
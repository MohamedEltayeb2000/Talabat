<!doctype html>
<html lang="en">

<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <meta name="description" content="">
  <meta name="author" content="Mark Otto, Jacob Thornton, and Bootstrap contributors">
  <meta name="generator" content="Hugo 0.79.0">
  <title>About US</title>

  <link rel="canonical" href="https://getbootstrap.com/docs/5.0/examples/carousel/">
  <script src="fontawesome-free-5.15.1-web/js/all.js"></script>



  <!-- Bootstrap core CSS -->
  <link href="css/bootstrap.min.css" rel="stylesheet">

  <style>
    body {
      background-color: #fff;
    }

    a {
      text-decoration: none;
    }

    .centered {
      position: absolute;
      top: 50%;
      left: 50%;
      transform: translate(-50%, -50%);
    }

    .bg h1 {
      font-size: 200%
    }

    .bg {
      position: relative;
      text-align: center;
      color: white;
      width: 100%;
      height: 100%;
      margin-bottom: 5rem;
      font-size: 200%;
    }

    /* MARKETING CONTENT
-------------------------------------------------- */

    /* Center align the text within the three columns below the carousel */
    .marketing .col-lg-4 {
      margin-bottom: 1.5rem;
      text-align: center;
    }

    .marketing h2 {
      font-weight: 400;
    }

    /* rtl:begin:ignore */
    .marketing .col-lg-4 p {
      margin-right: .75rem;
      margin-left: .75rem;
    }

    /* rtl:end:ignore */


    /* Featurettes
------------------------- */

    .featurette-divider {
      margin: 5rem 0;
      /* Space out the Bootstrap <hr> more */
    }

    /* Thin out the marketing headings */
    .featurette-heading {
      font-weight: 300;
      line-height: 1;
      /* rtl:remove */
      letter-spacing: -.05rem;
    }


    /* RESPONSIVE CSS
-------------------------------------------------- */

    @media (min-width: 40em) {

      /* Bump up size of carousel content */
      .carousel-caption p {
        margin-bottom: 1.25rem;
        font-size: 1.25rem;
        line-height: 1.4;
      }

      .featurette-heading {
        font-size: 50px;
      }
    }

    @media (min-width: 62em) {
      .featurette-heading {
        margin-top: 7rem;
      }
    }

    .bd-placeholder-img {
      font-size: 1.125rem;
      text-anchor: middle;
      -webkit-user-select: none;
      -moz-user-select: none;
      user-select: none;
    }

    @media (min-width: 768px) {
      .bd-placeholder-img-lg {
        font-size: 3.5rem;
      }
    }

    .col-md-9 {
      margin: 0 auto;
      /* Added */
      float: none;
      /* Added */
      margin-bottom: 5rem;
    }

    .card-body {
      margin: 1.5rem;

    }

    .card-body p+h1 {
      font-weight: bolder;

    }

    .card-body img {
      display: block;
      margin-top: 3rem;
      margin-bottom: 3rem;
    }

    .card-body .row {
      margin: auto;
      text-align: center;

    }
  </style>


  <!-- Custom styles for this template -->
  <link rel="stylesheet" href="css/chefs.css">
  <!-- <link href="abou.css" rel="stylesheet"> -->
  <script src="https://kit.fontawesome.com/a076d05399.js"></script>
</head>

<body>

  <header>
    <nav style="position:relative">
      <input type="checkbox" id="check" onclick="">
      <label for="check" class="checkbtn">
        <i class="fas fa-bars"></i>
      </label>
      <label class="logo" onclick="location.href='index.php';">HomeChef</label>
      <ul>
        <li><a class="active" href="index.php">Home</a></li>
        <li><a href="chefs.php">Restaurants</a></li>
        <?php
        session_start();
        if(!isset($_SESSION["type"])){
          echo "<li><a target=\"_blank\" href=\"join-us.html\">Join Us</a></li>\n<li><a href=\"login.html\">Login</a></li>";
        }
        else{
          if($_SESSION["type"] == "chef"){
            echo "<li><a href=\"chef.php?id=". $_SESSION["userInfo"]["data"]["id"] ."\">View account</a></li>\n<li><a href=\"chef.php?edit\">Edit account details</a></li>";
          }
          echo "<li><a href=\"chat.php\">Chat</a></li>\n<li><a href=\"cart.php\">Cart</a></li>\n<li><a href=\"logout.php\">Logout</a></li>
          <link href=\"css/navtry.css\" rel=\"stylesheet\">";
        }
        ?>
      </ul>
    </nav>
  </header>

  <main>

    <div class="bg">
      <img src="img/topbg.jpg" alt="topbg" class="responsive" style="width:100%;">
      <div class="centered">
        <h1 itemprop="headline" style="font-size:6vw;">About Us</h1>
        <span style="font-size:3vw;">A Greate Restaurant Website</span>
      </div>
    </div>
    <div class="col-md-9">
      <div class="card mb-3">
        <div class="card-body">
          <h1 class="title3" itemprop="headline" style="text-align:center">We Are Providing Food Online Service</h1>
          <img src="img/about-img.jpg" alt="about-img.jpg" itemprop="image" class="img-fluid mx-auto">
          <h1 class="sudo-bottom sudo-width70 sudo-bg-yellow" itemprop="headline"
            style="text-align:center; margin-bottom: 3rem;">Easy 3 Step Order</h1>
          <div class="row">
            <div class="col-lg-4">
              <img src="img/setp-img1.png"
                style="margin:auto; transform: scale(1.2); margin-bottom: 1rem; margin-top: 1rem">

              <h3 style="font-weight: bolder;">Explore Chifs</h3>
              <p><small class="text-muted">Find Best Chefs In Town By Just a little Click</small></p>
            </div><!-- /.col-lg-4 -->
            <div class="col-lg-4">
              <img src="img/setp-img2.png"
                style="margin:auto; transform: scale(1.2); margin-bottom: 1rem; margin-top: 1rem;">

              <h3 style="font-weight: bolder;">Choose a Tasty Dish</h3>
              <p><small class="text-muted"> Find Deals, Free Delivery, And More From Our Chefs Partners</small></p>
            </div><!-- /.col-lg-4 -->
            <div class="col-lg-4">
              <img src="img/setp-img3.png"
                style="margin:auto; transform: scale(1.2); margin-bottom: 1rem; margin-top: 1rem;">

              <h3 style="font-weight: bolder;">Make An Order</h3>
              <p><small class="text-muted">Customize Your Order Completely By Chatting With The Chef Himself</small></p>
            </div><!-- /.col-lg-4 -->
          </div><!-- /.row -->
        </div>
      </div>
    </div>
    <div class="container marketing">
      <hr class="featurette-divider">
      <div class="row featurette">
        <div class="col-md-7">
          <h2 class="featurette-heading">What Makes US So Special?
          </h2>
          <p class="lead">HomeChef tends to help people especially, Talented one to get a good income from a hobby, an
            interest, or a profession in cooking.
            This site try to present chefs to all the world and show their work. and also to help people getting a
            homemade clean healthy food.</p>
        </div>
        <div class="col-md-5">
          <img src="img/thai-food-takeout.jpg" width=500 height=500 class="img-fluid" alt="">

        </div>
      </div>

      <hr class="featurette-divider">

      <div class="row featurette">
        <div class="col-md-7 order-md-2">
          <h2 class="featurette-heading">Got A Special Request??</h2>
          <p class="lead">Don't Hesitate for a second, customize your own meal by chatting with the chef.
            you'r the champion of our story and we care about you.
          </p>
        </div>
        <div class="col-md-5 order-md-1">
          <img src="img/rib-eye-steak-in-chefs-hands.jpg" width=500 height=500 class="img-fluid" alt="">

        </div>
      </div>

      <hr class="featurette-divider">

      <div class="row featurette">
        <div class="col-md-7">
          <h2 class="featurette-heading">Got A Talent In Cooking?</h2>
          <p class="lead">Stop Wasting Your Time and hiding your talent, join us now and share your food with the world
            and make money with us. </p>
        </div>
        <div class="col-md-5">
          <img src="img/a-woman-in-her-shop.jpg" width=500 height=500 class="img-fluid" alt="">

        </div>
      </div>

      <hr class="featurette-divider">

      <!-- /END THE FEATURETTES -->

    </div><!-- /.container -->


    <!-- FOOTER -->
    <footer id="footer" class="footer-1">
      <div class="main-footer widgets-dark typo-light">
        <div class="container">
          <div class="row">

            <div class="col-xs-12 col-sm-6 col-md-3">
              <div class="widget subscribe no-box">
                <h5 class="widget-title">COMPANY NAME<span></span></h5>
                <p>About the company, little discription will goes here.. </p>
              </div>
            </div>

            <div class="col-xs-12 col-sm-6 col-md-3">
              <div class="widget no-box">
                <h5 class="widget-title">Quick Links<span></span></h5>
                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="#.">Get
                    Started</a></span><br>
                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="#.">Top
                    Leaders</a></span><br>
                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="#.">Success
                    Stories</a></span><br>
                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;"
                    href="#.">Event/Tickets</a></span><br>
                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="#.">News</a></span><br>
                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;"
                    href="#.">Lifestyle</a></span><br>
                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="Aboutus.php">About</a></span><br>
              </div>
            </div>

            <div class="col-xs-12 col-sm-6 col-md-3">
              <div class="widget no-box">
                <h5 class="widget-title">Get Started<span></span></h5>
                <p>Get access to your full Training and Marketing Suite.</p>
                <a class="btn" href="#." target="_blank">Register Now</a>
              </div>
            </div>

            <div class="col-xs-12 col-sm-6 col-md-3">

              <div class="widget no-box">
                <h5 class="widget-title">Contact Us<span></span></h5>

                <p><a href="mailto:info@domain.com" title="glorythemes">info@domain.com</a></p>
                <a title="youtube" target="_blank" href="https://www.youtube.com/"><img alt="youtube" width="30"
                    height="30"
                    src="data:image/png;base64,"></a>

                <a href="https://www.facebook.com/" target="_blank" title="Facebook"><img alt="Facebook" width="30"
                    height="30"
                    src="data:image/png;base64,"></a>

                <a href="https://twitter.com" target="_blank" title="Twitter"><img alt="Twitter" width="30" height="30"
                    src="data:image/png;base64,"></a>

                <a title="instagram" target="_blank" href="https://www.instagram.com/"><img alt="instagram" width="30"
                    height="30"
                    src="data:image/png;base64,"></a>

              </div>
            </div>

          </div>
        </div>
      </div>

      <div class="footer-copyright">
        <div class="container">
          <div class="row">
            <div class="col-md-12 text-center">
              <p>Copyright Company Name © 2016. All rights reserved.</p>
            </div>
          </div>
        </div>
      </div>
    </footer>
  </main>


  <script src="../assets/dist/js/bootstrap.bundle.min.js"></script>


</body>

</html>
<?php
session_start();
if (isset($_GET["remove"])) {
  array_splice($_SESSION["cart"], $_GET["remove"], 1);
  header("Location: cart.php");
}
?>
<!doctype html>
<html lang="en">

<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <meta name="description" content="">
  <meta name="author" content="Mark Otto, Jacob Thornton, and Bootstrap contributors">
  <meta name="generator" content="Hugo 0.79.0">
  <title>your cart</title>

  <link rel="canonical" href="https://getbootstrap.com/docs/5.0/examples/album/">
  <link href="css/chefs.css" rel="stylesheet">
  <link href="css/bootstrap.min.css" rel="stylesheet">
  <link rel="stylesheet" href="fontawesome-free-5.15.1-web/css/all.css">
  <script src="fontawesome-free-5.15.1-web/js/all.js"></script>
  <link href="css/chefs.css" rel="stylesheet">

  <style>
    * {
      font-family: montserrat;

    }

    a {
      text-decoration: none;
    }

    body {
      background: #eecda3;
      background: -webkit-linear-gradient(to right, #eecda3, #ef629f);
      background: linear-gradient(to right, purple, #1b9bff);
      min-height: 100vh;
    }

    #cart {
      margin-top: 80px;
    }

    #cart .row {
      display: grid;
    }

    .quantity {
      display: inline-block;
    }

    .quantity .input-text.qty {
      width: 35px;
      height: 39px;
      padding: 0 5px;
      text-align: center;
      background-color: transparent;
      border: 1px solid #efefef;
    }

    .quantity.buttons_added {
      text-align: left;
      position: relative;
      white-space: nowrap;
      vertical-align: top;
    }

    .quantity.buttons_added input {
      display: inline-block;
      margin: 0;
      vertical-align: top;
      box-shadow: none;
    }

    .quantity.buttons_added .minus,
    .quantity.buttons_added .plus {
      padding: 7px 10px 8px;
      height: 41px;
      background-color: #ffffff;
      border: 1px solid #efefef;
      cursor: pointer;
    }

    .quantity.buttons_added .minus {
      border-right: 0;
    }

    .quantity.buttons_added .plus {
      border-left: 0;
    }

    .quantity.buttons_added .minus:hover,
    .quantity.buttons_added .plus:hover {
      background: #eeeeee;
    }

    .quantity input::-webkit-outer-spin-button,
    .quantity input::-webkit-inner-spin-button {
      -webkit-appearance: none;
      -moz-appearance: none;
      margin: 0;
    }

    .quantity.buttons_added .minus:focus,
    .quantity.buttons_added .plus:focus {
      outline: none;
    }

    .btn.btn-primary.btn-lg {
      background-color: black;
      border-color: transparent;
    }

    .btn.btn-primary.btn-lg:hover {
      background-color: #1b9bff;
    }

    .mb-3 {
      display: grid;
    }

    @media(max-width:500px) {
      #cart {
        margin-top: 0;
        margin-bottom: -100px;
        margin-left: -12px;
      }
    }
  </style>


</head>

<body>

  <header>
    <nav style="position:relative">
      <input type="checkbox" id="check" onclick="">
      <label for="check" class="checkbtn">
        <i class="fas fa-bars"></i>
      </label>
      <label class="logo" onclick="location.href='index.php';">HomeChef</label>
      <ul>
        <li><a class="active" href="index.php">Home</a></li>
        <li><a href="chefs.php">Restaurants</a></li>
        <?php
        if (!isset($_SESSION["type"])) {
          echo "<li><a target=\"_blank\" href=\"join-us.html\">Join Us</a></li>\n<li><a href=\"login.html\">Login</a></li>";
        } else {
          if ($_SESSION["type"] == "chef") {
            echo "<li><a href=\"chef.php?id=" . $_SESSION["userInfo"]["data"]["id"] . "\">View account</a></li>\n<li><a href=\"chef.php?edit\">Edit account details</a></li>";
          }
          echo "<li><a href=\"chat.php\">Chat</a></li>\n<li><a href=\"cart.php\">Cart</a></li>\n<li><a href=\"logout.php\">Logout</a></li>
          <link href=\"css/navtry.css\" rel=\"stylesheet\">";
        }
        ?>
      </ul>
    </nav>
  </header>

  <main>
    <div class="pb-5">
      <div class="container" id="cart">
        <div class="row">
          <div class="col-lg-12 p-5 bg-white rounded shadow-sm mb-5">

            <!-- Shopping cart table -->
            <form method="post" action="backend/placeorder.php">
              <div class="table-responsive">
                <table class="table">
                  <thead>
                    <tr align="center">
                      <th scope="col" class="border-0 bg-light">
                        <div class="p-2 px-3 text-uppercase">Product</div>
                      </th>
                      <th scope="col" class="border-0 bg-light">
                        <div class="py-2 text-uppercase">Price</div>
                      </th>
                      <th scope="col" class="border-0 bg-light">
                        <div class="py-2 text-uppercase">Quantity</div>
                      </th>
                      <th scope="col" class="border-0 bg-light">
                        <div class="py-2 text-uppercase">Remove</div>
                      </th>
                    </tr>
                  </thead>


                  <tbody>
                    <?php
                    $sum = 0;
                    if (!isset($_SESSION["cart"])) {
                      $_SESSION["cart"] = [];
                    }
                    for ($i = 0; $i < count($_SESSION["cart"]); $i++) {
                      $price = (int)$_SESSION["cart"][$i]["price"];
                      $offer = (int)$_SESSION["cart"][$i]["offer"];
                      $new_price = $price - $price * $offer / 100;
                      $sum += $price - $price * $offer / 100;
                      echo "
				        	<tr>
		                    <td align='left' scope='row' style='width:30%''>
		                      <div class='p-2'>

		                        <img src=" . $_SESSION["cart"][$i]["image"] . " alt='' width='140' class='img-fluid rounded'>
		                        
		                        <div class='ml-3 d-inline-block align-middle'>
		                          <p class='mb-0' style='font-size: larger; font-weight: bolder;'>
		                           " . $_SESSION["cart"][$i]["name"] . "
		                        </p>
		                            <span class='text-muted font-weight-normal font-italic d-block'>
		                            	" . $_SESSION["cart"][$i]["restname"] . "
		                            </span>
		                        </div>
		                      </div>
		                    </td>
		                    <td align='center' class='align-middle'>
		                    	<strong class='price'>" . $new_price . "$</strong>
		                    </td>
		                    <td align='center' class='align-middle'>
		                      <div class='quantity buttons_added'>
		                        <input type='button' value='-' id='$i' class='minus' onclick=\"this.parentNode.querySelector('input[type=number]').stepDown();\">
	                          <input type='number' id='$i' step='1' min='1' max='' name='$i' value='1' title='Qty' class='input-text qty text quantitiy' size='4' pattern='' inputmode=''>
	                          <input type='button' value='+' id='$i' class='plus' onclick=\"this.parentNode.querySelector('input[type=number]').stepUp();\">
		                      </div>
		                    </td>
		                    <td align='center' class='align-middle'>
		                    	<a href='Cart.php?remove=$i' class='text-dark'>
		                    		<i class='fa fa-trash'></i>
		                    	</a>
		                    </td>
		                  </tr>";
                    }
                    ?>
                    <tr>
                      <td class="border-0">
                        <strong style="font-size:large">The total amount of your order</strong>
                      </td>
                      <td class="border-0"></td>
                      <td class="border-0"></td>
                      <td class="border-0" align="center" valign="middle"> <span><strong id="sum"><?php echo $sum . "$" ?></strong></span></td>
                    </tr>
                  </tbody>
                </table>
                <script type="text/javascript">
              var qunant = document.getElementsByClassName("quantitiy");
              var plus_btns = document.getElementsByClassName("plus");
              var minus_btns = document.getElementsByClassName("minus");
              for (var i = plus_btns.length - 1; i >= 0; i--) {
                plus_btns[i].addEventListener("mouseup", updatePricePlus);
                minus_btns[i].addEventListener("mouseup", updatePriceMinus);
                qunant[i].addEventListener("change", onEnterQuantity);
              }
              function updatePricePlus(){
                let quantitiy = parseInt(document.getElementsByName(this.id)[0].value);
                let price = parseFloat(document.getElementsByClassName("price")[this.id].innerHTML.split('$')[0]);
                let new_price = (quantitiy + 1) * price;
                let old_total = parseFloat(document.getElementById("sum").innerHTML.split('$')[0]);
                let total = old_total - price * quantitiy + new_price;
                document.getElementById("sum").innerHTML = Number(Math.round(total + "e" + 2) + "e-" +2) + "$";
              }

              function updatePriceMinus(){
                let quantitiy = parseInt(document.getElementsByName(this.id)[0].value);
                if(quantitiy-1 > 0){
                  let price = parseFloat(document.getElementsByClassName("price")[this.id].innerHTML.split('$')[0]);
                  let new_price = (quantitiy - 1) * price;
                  let old_total = parseFloat(document.getElementById("sum").innerHTML.split('$')[0]);
                  let total = old_total - price * quantitiy + new_price;
                  document.getElementById("sum").innerHTML = Number(Math.round(total + "e" + 2) + "e-" +2) + "$";
                }
              }
              function onEnterQuantity(e){
                let quantitiy = parseInt(this.value);
                if(quantitiy > 0){
                  let price = parseFloat(document.getElementsByClassName("price")[this.id].innerHTML.split('$')[0]);
                  let new_price = quantitiy * price;
                  let old_total = parseFloat(document.getElementById("sum").innerHTML.split('$')[0]);

                  let total = 0;
                  let qauntities = document.getElementsByClassName("quantitiy");

                  for (var i = qauntities.length - 1; i >= 0; i--) {
                    if(qauntities[i] != this){
                      total = parseFloat(document.getElementsByClassName("price")[qauntities[i].id].innerHTML.split('$')[0]) * parseInt(qauntities[i].value);
                    }
                  }

                  total =  total + new_price;
                  document.getElementById("sum").innerHTML = Number(Math.round(total + "e" + 2) + "e-" +2) + "$";
                }
              }
                </script>
              </div>
              <!-- End -->
              <button style="float:right; margin-top: 30px; width:100%" type="submit" class="btn btn-primary btn-lg">Check
                Out</button>
            </form>
          </div>
        </div>
      </div>
    </div>
    <footer id="footer" class="footer-1">
      <div class="main-footer widgets-dark typo-light">
        <div class="container">
          <div class="row">

            <div class="col-xs-12 col-sm-6 col-md-3">
              <div class="widget subscribe no-box">
                <h5 class="widget-title">HomeChef<span></span></h5>
                <p>HomeChef tends to help people especially, Talented one to get a good income from a hobby, an interest, or a profession in cooking. This site try to present chefs to all the world and show their work. and also to help people getting a homemade clean healthy food.</p>
              </div>
            </div>

            <div class="col-xs-12 col-sm-6 col-md-3">
              <div class="widget no-box">
                <h5 class="widget-title">Quick Links<span></span></h5>
                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="index.php">Home</a></span><br>
                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="chefs.php">Restaurants
                  </a></span><br>
                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="join-us.html" target="_blank">Join Us</a></span><br>
                <span class="thumb-content" style="float:left"><a style="color: #1b9bff;" href="Aboutus.php">About Us</a></span><br>
              </div>
            </div>

            <div class="col-xs-12 col-sm-6 col-md-3">
              <div class="widget no-box">
                <h5 class="widget-title">Get Started<span></span></h5>
                <p>Stop Wasting Your Time and hiding your talent, join us now and share your food with the world and make money with us.</p>
                <a style="color:white" class="btn" href="join-us.html" target="_blank">Join Us Now</a>
              </div>
            </div>

            <div class="col-xs-12 col-sm-6 col-md-3">

              <div class="widget no-box">
                <h5 class="widget-title">Contact Us<span></span></h5>

                <p><a href="mailto:info@domain.com" title="glorythemes">Gergesabdo99@gmail.com</a></p>
                <a title="youtube" target="_blank" href="https://www.youtube.com/"><img alt="youtube" width="30" height="30" src="data:image/png;base64,"></a>

                <a href="https://www.facebook.com/" target="_blank" title="Facebook"><img alt="Facebook" width="30" height="30" src="data:image/png;base64,"></a>

                <a href="https://twitter.com" target="_blank" title="Twitter"><img alt="Twitter" width="30" height="30" src="data:image/png;base64,"></a>

                <a title="instagram" target="_blank" href="https://www.instagram.com/"><img alt="instagram" width="30" height="30" src="data:image/png;base64,"></a>
              </div>
            </div>

          </div>
        </div>
      </div>

      <div class="footer-copyright">
        <div class="container">
          <div class="row">
            <div class="col-md-12 text-center">
              <p>Copyright HomeChef © 2021. All rights reserved.</p>
            </div>
          </div>
        </div>
      </div>
    </footer>

  </main>

  <script src="js/bootstrap.bundle.min.js"></script>

</body>

</html>